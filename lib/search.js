...
Search.prototype.queryTable = function(tokens, params, data, worker) {
  ...
  let wordConditions = [];
  let rankSelection = [];
  let strictConditions = "frecency > 250";
  ...
  let pendingTF = stmt.executeAsync({
    handleResult: function(aResultSet) {
      for (let row = aResultSet.getNextRow(); row; row=aResultSet.getNextRow()) {
        ...
        let revHost = row.getResultByName("rev_host");
        if (revHost in seenHosts)
          continue
        seenHosts[revHost] = true;
        let url = row.getResultByName("url");
        let isHub = helper.isURLHub(url);
        if (!isHub)
          continue
    ...
    handleCompletion: function(aReason) {
      //console.log("posting completion")
      try{
      worker.postMessage({
        "action": "display",
        "results" : results, //helper.addFallBacks("",results),
        "append" : data.append,
        "time"   : data.time,
        "title": data.title,
      });
 ...
 */
Search.prototype.search = function(tokens, params, data, worker) {
  let me = this;
