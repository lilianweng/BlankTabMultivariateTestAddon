...
function addFallBacks(currentURL, currentList, numResults) {
  ...
  // take care of blocked and pinned sites
  ...
  let numLimit = (16-currentList.length) + blockedSitesLength + pinnedSitesLength + 50;
  console.log("numLimit: "+numLimit);
  utils.spinQuery(Places.PlacesUtils.history.DBConnection, {
    "query" : "SELECT * FROM moz_places ORDER BY frecency DESC LIMIT " + numLimit,
    "params" : params,
    "names" : ["url", "title", "rev_host"],
  }).forEach(function({url, title, rev_host}) {
    if (count >= numResults || rev_host in seenHosts || !helper.isWebURL(url) || blockedSites[url] == true || pinnedSites[url] == true)
      return;
    currentList.push({
      "url" : url,
      "icon" : null,
