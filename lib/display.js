...
DisplaySites.prototype.loadResults = function(numResults, results, doc) {
  ...
  for(let i=0; i<results.length && i<numResults; i++) {
    ...
    $('newblanktabtestaddon-content').appendChild(elem);
  }

  if (noResults) {
    $('newblanktabtestaddon-no-results').style.display = 'block';
  }
  
  // clean all blocked sites
  $("newblanktabtestaddon-clean-blocked").addEventListener("click", function(event){
	  ss.storage._blockedSites = {};
  }, false);
...
DisplaySites.prototype.createSiteItem = function(result) {
  ...
  imageItem.setAttribute("src", fav.getFaviconData(result.url) );
  ...
  let color = fav.getCachedFaviconColor(imageItem);
  if (color) {
    colorBackground(imageContainer, color);
  } else {
    imageItem.addEventListener("load", function() {
        color = fav.getFaviconColor(this, doc);
        colorBackground(imageContainer, color);
    }, false);
  }
  ...
  if (result.pinned) {
    urlItem.setAttribute("name", "pinned");
    imageContainer.setAttribute("name", "pinned");
  ...
  if(result.fallback == true){
    urlItem.setAttribute("name", "fallback");
    imageContainer.setAttribute("name", "fallback");
  }
